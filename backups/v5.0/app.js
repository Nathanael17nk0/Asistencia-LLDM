...
function loadWeeklyTable() {
    ...
    if (sortedDates.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">No hay horarios cargados. Sube una foto o Excel.</td></tr>';
        return;
    }
    ...
    // Show dates >= today (or all?)
    // Let's show all for now so they see if Excel worked.
...
function showUserHistory(user) {
    const log = JSON.parse(localStorage.getItem('nexus_attendance_log') || '[]');
    const history = log.filter(e => e.userId === user.phone);
...
function initAdminFeatures() {
    ...
    if (setLocBtn) {
        setLocBtn.addEventListener('click', () => {
            if (STATE.currentLocation.lat === 0) {
                alert("⚠️ No se tiene ubicación actual. Asegúrate de permitir el GPS.");
                return;
            }
            const confirmSet = confirm(`¿Fijar esta ubicación como la Iglesia?\n\nLat: ${STATE.currentLocation.lat}\nLng: ${STATE.currentLocation.lng}`);
            if (confirmSet) {
                ...
                if (locStatus) {
                    locStatus.textContent = `✅ Fijado: ${STATE.currentLocation.lat.toFixed(4)}, ...`;
                    locStatus.style.color = 'green';
                }
                alert("✅ Ubicación de la iglesia actualizada.");
            }
        });
    }
    ...
    // 4. View Schedule Logic (New Button)
    ...
    if (adminSchedBtn) {
        adminSchedBtn.addEventListener('click', () => {
            ...
            // Ensure table is refreshed if needed
            if (typeof loadWeeklyTable === 'function') loadWeeklyTable();
        });
    }
...
function initApp() {
    ...
    if (modified) {
        localStorage.setItem('nexus_users', JSON.stringify(users));
        console.log("Migrated Users with missing IDs.");
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (fingerprintBtn) {
        fingerprintBtn.addEventListener('click', () => {
            if (!STATE.user) return showLogin();
            // ... (rest of legacy logic, simplified for brevity here, but assuming it calls other functions)
            // Ideally we keep the logic here or calls a named function
            // 1. Detect Service Slot
            const now = new Date();
            const { slotId, slotName } = getServiceSlot(now);
            ...
            // 3. Geolocation Check // DISABLED FOR ADMIN DEBUG
...
// INIT APP ON LOAD
// document.addEventListener('DOMContentLoaded', startApp); // REMOVED (Logic moved to global listener)
